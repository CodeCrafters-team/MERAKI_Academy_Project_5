...
export default function CourseDetailsClient({
  ...
  // تبويب نشط بالـ React بدل Bootstrap JS
  const [activeTab, setActiveTab] = useState<
    "overview" | "curriculum" | "instructor" | "reviews" | "quiz"
  >("overview");
    ...
    axios
      ...
      .then((res) => {
        ...
        const found = list.find(
          (c: any) => Number(c.course_id) === Number(course.id)
        );
        setCert(found || null);
      })
      .catch(() => setCert(null));
      ...
      axios
        ...
        .then((res) => {
          const lessons: Lesson[] = (res.data?.data || []).map((x: any) => ({
            ...
            title: x.title,
            duration_sec:
              x.duration_sec == null ? null : Number(x.duration_sec),
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE}/enrollments/check/${course.id}`, {
        headers: authHeader(),
      })
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE}/progress/course/${course.id}`, {
        headers: authHeader(),
      })
      .then((res) => {
  ...
  const totalDuration = Object.values(lessonsByModule)
    .flat()
    .reduce((s, l) => s + (l.duration_sec || 0), 0);
          ...
          setReviews((prev) =>
            prev.map((x) =>
              x.id === editId
                ? {
                    ...x,
                    rating: Number(d.rating ?? x.rating),
                    comment: d.comment ?? x.comment,
                  }
                : x
            )
          );
  ...
  useEffect(() => {
    if (progress.progressPercent >= 100 && activeTab === "overview") {
    }
  }, [progress.progressPercent, activeTab]);

  return (
    <div className="container py-4 animate__animated animate__fadeIn">
      <div className="mb-3 animate__animated animate__fadeInDown">
        <h1 className="fw-bold mb-1 animate__animated animate__fadeInDown">
          {course.title}
        </h1>
        <p className="text-muted mb-2 animate__animated animate__fadeInUp">
          {course.description}
        </p>
      ...
      <div className="row g-4">
        <div className="col-lg-4 border animate__animated animate__fadeInUp animate__delay-1s">
          <div
            className="card shadow-sm mb-3 p-2 animate__animated animate__fadeInUp"
            style={{ marginTop: "2.55rem" }}
          >
            <div
              className="animate__animated animate__fadeIn"
              style={{ height: "100%", background: "#eef4fb" }}
            >
              {course.cover_url ? (
                <img
                ...
                <span className="d-flex align-items-center justify-content-center h-100 text-muted animate__animated animate__fadeIn">
                  No cover
                </span>
              )}
            </div>
            <div className="card-body animate__animated animate__fadeInUp">
              {checkingEnroll ? (
                ...
                <>
                  <h3
                    style={{ color: "green" }}
                    className="fw-bold animate__animated animate__fadeInDown"
                  >
              ...
              <div className="d-flex align-items-center gap-2 animate__animated animate__fadeIn">
                {course.avatar_url ? (
                  <img
                    ...
                    alt=""
                    className="rounded-circle animate__animated animate__zoomIn"
                    style={{ width: 40, height: 40, objectFit: "cover" }}
                  />
                ) : (
                  <div
                    className="rounded-circle animate__animated animate__fadeIn"
                    style={{ width: 40, height: 40, background: "#e9ecef" }}
                  />
                )}
                <div>
                  <div className="fw-semibold animate__animated animate__fadeInDown">
                    {course.first_name} {course.last_name}
                  </div>
                  <div className="small text-muted animate__animated animate__fadeInUp">
                    Instructor
                  </div>
                </div>
              </div>
              <button
                className="btn btn-primary btn-sm animate__animated animate__pulse border-0"
                onClick={goMessageInstructor}
              >
                Message
              </button>
            </div>
          ...
          <div className="card border shadow-sm animate__animated animate__fadeInUp mb-2 ">
            <div className="card-body">
              <h6 className="mb-2 animate__animated animate__fadeInDown">
          ...
          <div className="tab-content border border-top-0 p-3 rounded-bottom shadow-sm animate__animated animate__fadeInUp">
            {activeTab === "overview" && (
              <div className="tab-pane fade show active animate__animated animate__fadeIn">
                <h5 className="mb-2 animate__animated animate__fadeInDown">
                  About this course
                </h5>
                <p className="mb-0 animate__animated animate__fadeInUp">
                  {course.description}
                </p>
              </div>
            )}

            {progress.progressPercent >= 100 && activeTab === "quiz" && (
              <div className="tab-pane fade show active" id="quiz">
                <div className="p-3">
                  <h4 className="mb-3">
                ...
                {modules.map((m, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span>
                          {m.title} ({lessons.length} lessons)
                        </span>
                      </button>
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="d-flex align-items-center gap-2">
                                  {isEnrolled &&
                                    (isDone ? (
                                      <span className="badge bg-success animate__animated animate__zoomIn">
                                        Completed
                                      </span>
                                    ) : (
                                      <span className="badge bg-warning text-dark animate__animated animate__fadeInRight">
                                        Not completed
                                      </span>
                                    ))}
                                  ...
                                  {canWatch ? (
                                    ...
                                    <span className="badge bg-secondary animate__animated animate__fadeInRight">
                                      Locked
                                    </span>
                                  )}
                                </div>
            ...
            {activeTab === "instructor" && (
              <div className="tab-pane fade show active animate__animated animate__fadeIn" id="instructor">
                <div className="d-flex align-items-start gap-3 animate__animated animate__fadeInUp">
                  ...
                  <div className="animate__animated animate__fadeIn">
                    <div className="d-flex flex-wrap align-items-center gap-2">
                      <strong className="fs-5 animate__animated animate__fadeInDown">
                        {course.first_name} {course.last_name}
                      </strong>
                      {course.email && (
                        <span
                    ...
                    {course.bio && (
                      <p className="mt-2 mb-0 text-muted" style={{ whiteSpace: "pre-line" }}>
                        {course.bio}
                      </p>
                    )}
                  </div>
            ...
            {activeTab === "reviews" && (
              <div className="tab-pane fade show active animate__animated animate__fadeIn" id="reviews">
                ...
                <div className="mb-3 border rounded p-3 animate__animated animate__fadeInUp">
                  {auth.id ? (
                    <>
                      ...
                      <textarea
                        className="form-control mb-2 animate__animated animate__fadeIn"
                        value={addComment}
                    ...
                    <div className="text-muted mb-2 animate__animated animate__fadeIn">
                      Log in to add a review
                    </div>
                  )}
                  <button
                    className="btn btn-primary border-0 animate__animated animate__pulse"
                ...
                {reviews.map((r, i) => (
                  <div
                    key={r.id}
                    className="border rounded p-3 mb-2 animate__animated animate__fadeInUp"
                  ...
                  >
                    <div className="d-flex align-items-center gap-2 mb-1 animate__animated animate__fadeIn">
                      ...
                      <strong className="animate__animated animate__fadeInLeft">
                        {(r.first_name || "") + " " + (r.last_name || "")}
                      </strong>
                      <small className="text-muted ms-auto animate__animated animate__fadeInRight">
      ...
      {editOpen && (
        ...
        >
          ...
          >
            ...
            <textarea
              className="form-control mb-3 animate__animated animate__fadeIn"
              rows={3}
              value={editComment}
            ...
            <div className="d-flex justify-content-end gap-2">
              ...
              <button
                className="btn btn-primary animate__animated animate__pulse border-0"
